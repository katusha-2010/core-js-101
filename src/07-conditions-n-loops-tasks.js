...
function getSumBetweenNumbers(n1, n2) {
  // throw new Error('Not implemented');
  const arr = Array(Math.abs(n1 - n2) + 1).fill();
  const arr1 = arr.map((el, i) => i + n1);
  return arr1.reduce((p, c) => p + c);
}
...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const a = {
    x1: rect1.left,
    y1: rect1.top,
    x2: rect1.width + rect1.left,
    y2: rect1.height + rect1.top,
  };

  const b = {
    x1: rect2.left,
    y1: rect2.top,
    x2: rect2.width + rect2.left,
    y2: rect2.height + rect2.top,
  };

  if (a.x1 === a.x2 || a.y1 === a.y2 || b.x2 === b.x1 || b.y1 === b.y2) { return true; }
...
function isInsideCircle(circle, point) {
  ...
  if (((point.y - y1) ** 2 + (point.x - x1) ** 2) < circle.radius ** 2) { return true; }
  return false;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  // throw new Error('Not implemented');
  const orderNumb = [a, b].sort((k, l) => k - l).join(', ');
  const firstSymb = isStartIncluded === true ? '[' : '(';
  const lastSymb = isEndIncluded === true ? ']' : ')';
  return `${firstSymb}${orderNumb}${lastSymb}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  const newArr = arr.map((el, i) => {
    const examin = arr.length % 2 === 0 ? i % 2 === 0 : i % 2 !== 0;
    if (examin) {
      const doubledEl = el * 2;
      return doubledEl > 9 ? doubledEl - 9 : doubledEl;
    }
    return +el;
  });

  const sum = newArr.reduce((p, c) => p + c);
  return (sum % 10 === 0);
}
...
function getDigitalRoot(num) {
  // throw new Error('Not implemented');
  const sum = num.toString().split('').reduce((p, c) => +p + +c);
...
function getCommonDirectoryPath(arr) {
  ...
  for (let i = 0; i < arr[0].length; i += 1) {
    for (let j = 0; j < arr.length - 1; j += 1) {
      ...
      if (j === arr.length - 2) { result += arr[j][i]; }
    }
  }
...
function getMatrixProduct(m1, m2) {
  ...
  const result = Array(rows1).fill();
  for (let i = 0; i < rows1; i += 1) {
    result[i] = Array(col2).fill();
